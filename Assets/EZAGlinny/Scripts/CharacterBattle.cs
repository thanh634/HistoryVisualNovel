...
public class CharacterBattle : MonoBehaviour {
    ...
    public void Setup(Character.Type characterType, BattleHandler.LanePosition lanePosition, Vector3 startingPosition, bool isPlayerTeam, Character.Stats stats) {
        ...
        case Character.Type.EvilMonster_3:
            ...
            break;
        case Character.Type.Enemy_SwordShied1:
            material.mainTexture = GameAssets.i.t_Randy;
            playerBase.GetAnimatedWalker().SetAnimations(GameAssets.UnitAnimTypeEnum.dSwordShield_Idle, GameAssets.UnitAnimTypeEnum.dSwordShield_Walk, 1f, 1f);
            attackUnitAnimType = GameAssets.UnitAnimTypeEnum.dSwordShield_Attack;
            transform.localScale = Vector3.one * 1.0f;
            break;
        case Character.Type.Enemy_SwordShied2:
            material.mainTexture = GameAssets.i.t_Vendor;
            playerBase.GetAnimatedWalker().SetAnimations(GameAssets.UnitAnimTypeEnum.dSwordShield_Idle, GameAssets.UnitAnimTypeEnum.dSwordShield_Walk, 1f, 1f);
            attackUnitAnimType = GameAssets.UnitAnimTypeEnum.dSwordShield_Attack;
            transform.localScale = Vector3.one * 1.0f;
            break;
        case Character.Type.Enemy_SwordShied3:
        ...
        if (isPlayerTeam){
            healthWorldBar = new World_Bar(transform, healthWorldBarLocalPosition, new Vector3(12 * (stats.healthMax / 100f), 1.6f), Color.grey, Color.green, healthSystem.GetHealthPercent(), UnityEngine.Random.Range(1000, 2000), new World_Bar.Outline { color = Color.black, size = .6f });
        }
        else {
    ...
    public void Move(BattleHandler.LanePosition lanePosition, bool faceRight, Action onSlideComplete) {
        this.lanePosition = lanePosition;
        Vector3 targetPosition = BattleHandler.GetPosition(lanePosition, true);
        FlipCharacter(faceRight);
        SlideToPosition(targetPosition, onSlideComplete);
    }
    ...
    public int GetAttack(float attackMultiplier) {
        return (int) (stats.attack * attackMultiplier);
    }
