...
public class AudioChannel
{
    ...
    private const string TRACH_CONTAINER_NAME_FORMAT = "Channel - [{0}]";
    ...
    public AudioChannel(int channel)
    {
        ...
        trackContainer = new GameObject(string.Format(TRACH_CONTAINER_NAME_FORMAT, channel)).transform;
    ...
    private void SetAsActiveTrack(AudioTrack track)
    {
        ...
        activeTrack = track;
        //Debug.Log($"song {activeTrack.name}, start from {activeTrack.volume} to {activeTrack.volumeCap}");
    ...
    private IEnumerator VolumeLeveling()
    {
        ...
        {
            //Debug.Log($"active track : {activeTrack.name}, {activeTrack.volume} to {activeTrack.volumeCap}");
            ...
            for (int i = tracks.Count - 1; i >= 0; i--)
            {
                ...
                //Debug.Log($"track : {track.name}, {track.volume} to {track.volumeCap}");
