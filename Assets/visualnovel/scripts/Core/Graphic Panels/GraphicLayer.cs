...
public class GraphicLayer
{
    ...
    public Coroutine SetTexture(string filePath, float transitionSpeed = 1f, Texture blendingTexture = null, bool immediate = false)
    {
        ...
        if(tex == null )
        {
            Debug.LogError($"Could not load graphic texture from path '{filePath}, Please ensure it exists within Resources folder!");
            return null;
        }
    ...
    public Coroutine SetVideo(string filePath , float transitionSpeed = 1f, Texture blendingTexture = null, bool useAudio = true, bool immediate = false)
    {
        VideoClip clip = Resources.Load<VideoClip>(filePath);

        if (clip == null)
        {
            Debug.LogError($"Could not load graphic video from path '{filePath}, Please ensure it exists within Resources folder!");
            return null;
        }
    ...
    private Coroutine CreateGraphic<T>(T graphicData, float transitionSpeed = 1f, string filePath = "", bool useAudioForVideo = true, Texture blendingTexture = null, bool immediate = false)
    {
        ...
        switch(graphicData)
        {
            ...
            case VideoClip:
                ...
                break;
            default:
                Debug.Log("Graphic type undefined in CreateGraphic for GraphicLayer");
                return null;
        }
